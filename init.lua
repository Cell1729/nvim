...
vim.keymap.set('n', '<leader>fg', builtin.live_grep, { desc = 'ファイル内文字列検索 (grep)' })
vim.keymap.set('n', '<leader>fb', builtin.buffers, { desc = '開いているバッファ一覧' })
vim.keymap.set('n', '<leader>fh', builtin.help_tags, { desc = 'ヘルプタグを検索' })
vim.keymap.set('n', '<leader>fr', builtin.oldfiles, { desc = '最近使ったファイル (最近開いた順)' })
